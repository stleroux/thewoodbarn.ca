          ...
          <div class="col-md-12">
            <div class="panel panel-default">
                ...
                <div class="pull-right">
                  ...
                  @if(Auth::check())
                    @if($post->user_id == Auth::user()->id)
                      {{-- @ability('admin', 'admin,post_edit,post_edit_admin') --}}
                      {{-- @ability('', '') --}}
                        @role('admin')
                      ...
                      {{-- @endability --}}
                    @else
                      @ability('admin', 'admin,post_edit_admin')
                        <a href="{{ route('posts.edit', $post->id) }}" class="btn btn-info btn-xs" {{ (Auth::user()->actionButtons == 1) ? 'title=Edit' : '' }}>
                          @yield('edit_buttons')
                        </a>
                      @endability
                        ...
                        <form method="POST" action="{{ route('posts.destroy', $post->id) }}" accept-charset="UTF-8" style="display:inline">
                          ...
                          <button
                            ...
                            data-message="Are you sure you want to delete this post?">
                              @yield('delete_buttons')
                          </button>
                        </form>
                      @endability
                    @else
                      @ability('admin', 'admin,posts_delete_admin')
                        <form method="POST" action="{{ route('posts.destroy', $post->id) }}" accept-charset="UTF-8" style="display:inline">
                          <input type="hidden" name="_method" value="DELETE">
                          <input type="hidden" name="_token" value="{{ csrf_token() }}">
                          <button
              ...
              <div class="panel-footer">
                ...
                <div class="pull-right">
                  {{-- Published on @include('layouts.common.dateFormat', ['model'=>$post, 'field'=>'created_at']) --}}
                </div>
              </div>
            </div>
          </div>
